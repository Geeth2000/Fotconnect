...
// AcademicFragment displays a list of academic news articles in a RecyclerView.
...
public class AcademicFragment extends Fragment implements ArticleAdapter.OnReadMoreClickListener {
    ...
    private List<Article> articleList; // List to hold Article objects for academic news

    // Firebase Database Reference to the "academic" news section.
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        // Inflate the layout for this fragment (fragment_academic.xml).
        ...
        // to the "academic" child node under the "news" parent node.
        FirebaseDatabase database = FirebaseDatabase.getInstance();
        newsRef = database.getReference("news").child("academic");

        // Get an instance of Firebase Storage
        storage = FirebaseStorage.getInstance();

        // Call the method to fetch academic news from Firebase.
        fetchAcademicNews();

        return view;
    }

    // Fetches academic news data from Firebase Realtime Database.
    private void fetchAcademicNews() {
        ...
        newsRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                // Assuming "academic" node directly contains a single news item as per your JSON,
                // we directly map it to an Article object.
                final Article academicNews = dataSnapshot.getValue(Article.class);

                if (academicNews != null) {
                    // Check if the imageUrl is a gs:// URL (Firebase Storage URI)
                    if (academicNews.getImageUrl() != null && academicNews.getImageUrl().startsWith("gs://")) {
                        // Get a reference to the image in Firebase Storage using the gs:// URL
                        StorageReference imageRef = storage.getReferenceFromUrl(academicNews.getImageUrl());

                        // Get the public download URL asynchronously
                        imageRef.getDownloadUrl().addOnSuccessListener(uri -> {
                            // Update the Article object with the new public download URL
                            academicNews.setImageUrl(uri.toString());
                            // Add the article to the list and notify adapter
                            articleList.add(academicNews);
                    ...
                    // If no academic news is found, notify adapter (it will be empty)
                    adapter.notifyDataSetChanged();
                }
            }
            ...
            public void onCancelled(@NonNull DatabaseError databaseError) {
                ...
                Toast.makeText(getContext(), "Failed to load academic news: " + databaseError.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    public void onReadMoreClick(Article article) {
        ...
        // addToBackStack(null) allows the user to press the back button to return to AcademicFragment.
